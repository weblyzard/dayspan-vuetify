...
export default function DaySpanComponent() {
  this.version = '0.3.0';
  this.readOnly = false;
  ...
  this.now = Day.now();
  this.timeout = null;
  this.refreshInterval = Constants.MILLIS_IN_MINUTE;
  ...
  this.inactiveBlendTarget = { r: 255, g: 255, b: 255 };
  this.inactiveBlendAmount = 0.5;

  this.placeholderBlendAmount = 0.2;
  this.placeholderBlendTarget = { r: 255, g: 255, b: 255 };
  ...
  this.addLocales = (names, locale) => {
    names.forEach(n => this.addLocale(n, locale));
  };

  this.updateLocale = (name, update, strict = true) => {
    const locale = this.getLocale(name);
  ...
  this.copyEventDetails = (details) => {
    return dsMerge({}, details);
  };

  this.createEventData = (details) => {
    return dsMerge({}, details);
  };

  this.createEvent = (details, schedule) => {
    return Parse.event({
      schedule: schedule,
  ...
  this.getEventOccurrence = (schedule, start, labels, formats) => {
  ...
  this.getEventAgendaWhen = (calendarEvent, labels, formats) => {
    let when = '';
  ...
  this.getEventDuration = (schedule, labels) => {
  ...
  this.getPlaceholderEventDetails = () => {
  ...
  this.getPlaceholderEventForAdd = (time) => {
  ...
  this.getPlaceholderEventForMove = (original) => {
  ...
  this.getStyleFull = (details, calendarEvent, index) => {
    ...
    this.getStyleTimed = (details, calendarEvent) => {
  ...
  this.getStyleColor = (details, calendarEvent, past, cancelled) => {
  ...
  this.getStylePlaceholderTimed = (details, placeholder, forDay) => {
  ...
  this.getStylePlaceholderFull = (details, calendarEvent, index, forDay) => {
    // let color = this.getStyleColor(details, calendarEvent);
  ...
  this.clampComponent = (c) => {
    return Math.max(0, Math.min(255, Math.floor(c)));
  };

  this.clampColor = (color, out) => {
  ...
  this.stopRefreshTimes = () => {
    clearTimeout(this.timeout);

    this.timeout = null;
  };

  this.refreshTimes = (force = false) => {
    let today = Day.today();
